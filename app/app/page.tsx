...
export default function Home() {
  ...
  const currentReading = liveState.current_reading;
  const currentRaw = liveState.current_raw as IssData | null;
  const current = currentReading && currentRaw ? { reading: currentReading, raw: currentRaw } : null;
  ...
  const age = current ? Math.max(0, Math.floor((now - Date.parse(current.reading.source_time!)) / 1000)) : null;
  const stale = !!error || !current || (age !== null && age > 120);
  const ageLabel = age === null ? '관측 기록 대기' : age < 60 ? `${age}초 전` : age < 3600 ? `${Math.floor(age / 60)}분 전` : `${Math.floor(age / 3600)}시간 전`;
  const latestSaved = records[records.length - 1];
  const previousSaved = records.length > 1 ? records[records.length - 2] : null;
  const savedDelta = latestSaved ? compare(latestSaved.reading, previousSaved?.reading) : null;
  ...
  return <div className="app-shell">
    ...
    <main id="main">
      ...
      <div className={`status-strip ${error ? 'has-error' : ''}`} aria-live="polite"><span className="status-main">{error ? <WifiOff size={16} /> : <Radio size={16} />}<strong>{error ? ERRORS[error].title : !current ? '관측 기록 대기' : stale ? '저장된 관측 데이터' : live ? '새 데이터 수신 완료' : '첫날 관측 기록 확보'}</strong></span><span>{error ? ERRORS[error].action : !current ? '공개 보존 관측 기록이 없습니다. 새 데이터를 조회하면 현재 관측값을 표시합니다.' : `출처 기준 ${ageLabel} · ${stale ? '현재 위치와 다를 수 있습니다' : '공개 API 응답 기준'}`}</span><span className={`status-pill ${stale ? 'amber' : ''}`}><i className="dot" />{!current ? '기록 대기' : stale ? '오래된 값' : '최근 값'}</span></div>
      {error && <p className="error-detail">{ERRORS[error].detail} 이 표시는 앱의 데이터 수신 상태이며 ISS의 이상을 뜻하지 않습니다.</p>}

      <div className="observation-grid">
        {current ? <section className="panel map-panel" aria-labelledby="map-title"><div className="panel-heading"><div><span className="eyebrow">POSITION MONITOR</span><h2 id="map-title"><Globe2 size={17} />궤도 위의 한 점</h2></div><span className="subtle-tag">지상 위치</span></div><WorldMap raw={current.raw} stale={stale} /><div className="coordinates"><span><MapPin size={15} />관측 좌표</span><span>위도 <strong>{coordinate(current.raw.latitude, 'N', 'S')}</strong></span><span>경도 <strong>{coordinate(current.raw.longitude, 'E', 'W')}</strong></span></div></section> : <section className="panel map-panel" aria-labelledby="map-title"><div className="panel-heading"><div><span className="eyebrow">POSITION MONITOR</span><h2 id="map-title"><Globe2 size={17} />궤도 위의 한 점</h2></div><span className="subtle-tag">기록 대기</span></div><p className="empty-observation">표시할 위치 관측값이 없습니다. 새 데이터를 조회하면 지도와 좌표를 표시합니다.</p></section>}

        {current ? <section className="panel altitude-panel" aria-labelledby="altitude-title"><div className="panel-heading"><div><span className="eyebrow">PRIMARY SIGNAL</span><h2 id="altitude-title">지표면 위 고도</h2></div><Satellite size={22} className="mint" /></div><div className="altitude-number">{number(current.reading.normalized_value)}<span>km</span></div><p className="altitude-caption">ISS가 지구 표면에서 떨어진 높이</p><div className="comparison-chip"><Clock3 size={14} />{deltaLabel}</div><div className="height-diagram" aria-label="고도 개념도, 축척과 무관"><div className="diagram-level"><span>ISS</span><Satellite size={22} /><span>{number(current.raw.altitude)} km</span></div><div className="height-line"><i /><span>지구 저궤도</span></div><div className="earth-line"><span>지표면</span><span>0 km</span></div></div><div className="altitude-foot"><ShieldCheck size={15} /><span>마지막 정상값 보존</span><span className="tiny">개념도 · 축척 무관</span></div></section> : <section className="panel altitude-panel" aria-labelledby="altitude-title"><div className="panel-heading"><div><span className="eyebrow">PRIMARY SIGNAL</span><h2 id="altitude-title">지표면 위 고도</h2></div><Satellite size={22} className="mint" /></div><p className="empty-observation">표시할 고도 관측값이 없습니다. 공개 기록 또는 새 조회 결과가 생기면 원래 정밀도의 고도를 표시합니다.</p></section>}
      </div>

      <div className="telemetry-row">{current ? <><section><span className="metric-label"><Activity size={16} />비행 속도</span><p>{number(current.raw.velocity, 0)}<small>km/h</small></p><span className="metric-note">동일 응답의 순간 속도</span></section><section><span className="metric-label"><Clock3 size={16} />출처 기준 시각</span><p className="time-value">{new Intl.DateTimeFormat('en-GB', { timeZone: 'Asia/Seoul', hour: '2-digit', minute: '2-digit', second: '2-digit' }).format(new Date(current.reading.source_time!))}</p><span className="metric-note">{kstDate(current.reading.source_time!)} · Asia/Seoul</span></section><section><span className="metric-label"><Radio size={16} />조회 시각</span><p className="time-value">{new Intl.DateTimeFormat('en-GB', { timeZone: 'Asia/Seoul', hour: '2-digit', minute: '2-digit', second: '2-digit' }).format(new Date(current.reading.fetched_at))}</p><span className="metric-note">{current.reading.record_date} · Asia/Seoul</span></section><section><span className="metric-label"><Database size={16} />보존된 일별 기록</span><p>{String(records.length).padStart(2, '0')}<small>/ 02일</small></p><span className="metric-note">서로 다른 실제 한국 날짜</span></section></> : <><section><span className="metric-label"><Activity size={16} />비행 속도</span><p className="time-value">—</p><span className="metric-note">현재 관측값 대기</span></section><section><span className="metric-label"><Clock3 size={16} />출처 기준 시각</span><p className="time-value">—</p><span className="metric-note">현재 관측값 대기</span></section><section><span className="metric-label"><Radio size={16} />조회 시각</span><p className="time-value">—</p><span className="metric-note">현재 관측값 대기</span></section><section><span className="metric-label"><Database size={16} />보존된 일별 기록</span><p>{String(records.length).padStart(2, '0')}</p><span className="metric-note">서로 다른 실제 한국 날짜</span></section></>}</div>
