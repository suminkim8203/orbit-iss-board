...
test('public normal sequence matches every expected state and stable IDs', () => {
...
for (const name of ['timeout', 'auth-401', 'rate-429', 'offline', 'schema-break']) {
  test(`${name}: preserves last good row and recovers exactly once`, async () => {
...
test('KST midnight, not UTC midnight, controls the unique date', () => {
...
test('invalid normalized values are never stored', () => {
...
test('earlier response cannot overwrite newer same-day data', () => {
  assert.equal(runFixture(baseline(), a).current_reading.normalized_value, 105);
});
test('comparisons retain sign and flag missing yesterday or incompatible units', () => {
...
test('live pipeline preserves source precision and distinct source/fetch times', async () => {
...
test('live HTTP and parse failures have distinct codes', async () => {
...
test('Retry-After handles seconds, dates and malformed values', () => {
...
test('synthetic errors cannot alter the real ISS store', () => {
